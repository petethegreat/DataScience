...
```rstandard``` and ```rstudent``` are functions that normalise residuals



influence is measured by removing points from the dataset and rerunning the regression. ```dffits``` and ```dfbetas```

useful plot - plot the residuals vs the fitted values. 
